...
export const ListProvider = ({ children }) => {
    ...
    const getMyLists = async () =>{
        ...
        const listasQuery = query(listCollection, where('Owner', '==', userInformation.email));
        ...
        if (fetchedLists.size > 0){
            ...
            fetchedLists.forEach(doc=>{
                ...
                if (!lists?.myLists?.[doc.id]){
                    let auxLists = lists;
                    auxLists.myLists[doc.id] = { name: data.Nombre, owner: data.Owner, createdDate: data.CreatedDate.toDate() }
    ...
    return (
        ...
        >
            {authState.isLoading || lists.isLoading ? (
                <Loading/>
            ): (children)}
        </ListContext.Provider>
    )
