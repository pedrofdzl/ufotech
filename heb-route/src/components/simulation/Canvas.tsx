...
import { collection, getDocs } from "firebase/firestore";
  ...
  export default function Canvas({
    ...
    const drawImage = (num: number, x: number, y: number, tileSize: number, isPath: boolean) => {
    ...
    useEffect(() => {
      ...
      const init = async () => {
        ...
        function searchAll(startX: number, startY: number, i: number, j: number, x: number, y: number, visited: number[][], dist: number) {
        ...
        function dfs(startX: number, startY: number, i: number, j: number, visited: number[][], dist: number) {
          visited[i][j] = dist;
          for (let k = 0; k < 4; k++) {
            const x = i + delta[k];
            const y = j + delta[k + 4];
    ...
    const doZoom = useCallback((zoom: number) => {
      if (context) {
    ...
    const getMouseMove = ((event: (MouseEvent | React.MouseEvent<HTMLCanvasElement>) | (TouchEvent | React.TouchEvent<HTMLCanvasElement>)) => {
      let pageX: number;
      let pageY: number;
    ...
    function handleTouchStart(event: TouchEvent) {
      if (event.touches.length === 2) {
        ...
        (event.target as HTMLElement).dataset.initialDistance = initialDistance.toString();
      }
    }
    ...
    const handleUpdateMouse = useCallback((event: MouseEvent | TouchEvent) => {
      if (canvasRef.current) {
        if (event instanceof TouchEvent && event.touches.length === 2) {
          ...
          const initialDistance = parseInt((event.target as HTMLElement).dataset.initialDistance || '0');
      
          const scale = currentDistance / initialDistance;
  
          (event.target as HTMLElement).dataset.initialDistance = currentDistance.toString();
